...
esp_err_t LED::ledc_init(gpio_num_t gpio_num, uint8_t invert)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG_LED, "ledc_timer_config failed: %s", esp_err_to_name(err));
        return err;
    }
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG_LED, "ledc_channel_config failed: %s", esp_err_to_name(err));
        return err;
    }
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG_LED, "ledc_fade_func_install failed: %s", esp_err_to_name(err));
        return err;
    }
    ledc_cbs_t callbacks = {
        .fade_cb = LED::fade_cb,
    };
    err = ledc_cb_register(ledc_channel.speed_mode, ledc_channel.channel, &callbacks, this);
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG_LED, "ledc_cb_register failed: %s", esp_err_to_name(err));
        return err;
    }
    return err;
}
...
esp_err_t LED::set_mode(uint8_t mode, uint8_t blink_code)
{
    ...
    if (mode != 1 && this->mode == 1)
    {
        ...
        if (err != ESP_OK)
        {
            ESP_LOGE(TAG_LED, "gpio_reset_pin failed: %s", esp_err_to_name(err));
            goto exit;
        }
        // init gpio
        err = led_gpio_init(gpio_num);
        if (err != ESP_OK)
        {
            ESP_LOGE(TAG_LED, "led_gpio_init failed: %s", esp_err_to_name(err));
            goto exit;
        }
    }
