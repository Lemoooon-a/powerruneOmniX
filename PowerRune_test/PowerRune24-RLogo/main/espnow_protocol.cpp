...
static void reset_armour_id_task(void *pvParameter)
{
...
// // 比较MAC地址，如果不是自己的MAC地址或者广播地址，直接丢弃
    // Accept unicast to self and broadcast frames on all roles.
    if (memcmp(esp_now_info->des_addr, self_mac, ESP_NOW_ETH_ALEN) != 0 &&
        memcmp(esp_now_info->des_addr, broadcast_mac, ESP_NOW_ETH_ALEN) != 0)
    {
...
// ID校验
            ...
            // Broadcast PING packets from other nodes are for peer discovery.
            // Ignore them locally to avoid polluting rx_id/ack flow.
            if ((espnow_data_pack->event_base[2] == 'A' && espnow_data_pack->event_id == PRA_PING_EVENT) ||
                (espnow_data_pack->event_base[2] == 'M' && espnow_data_pack->event_id == PRM_PING_EVENT))
            {
                free(received_data->data);
                free(espnow_data_pack);
                continue;
...
void ESPNowProtocol::tx_callback(const esp_now_send_info_t *tx_info, esp_now_send_status_t status)
{
    if (status == ESP_NOW_SEND_SUCCESS)
    {
        xEventGroupSetBitsFromISR(send_state, SEND_COMPLETE_BIT, &xHigherPriorityTaskWoken);
    ...
    else
    {
        ...
        if (tx_info && tx_info->des_addr)
            ESP_LOGE(TAG_MESSAGER, "Sending data to " MACSTR " failed", MAC2STR(tx_info->des_addr));
        else
            ESP_LOGE(TAG_MESSAGER, "Sending data failed (no dest mac)");
    }
}
...
void ESPNowProtocol::tx_event_handler(void *handler_args, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    if (peer_list_established)
    {
        ...
        if (memcmp(dest_mac, NULL_mac, ESP_NOW_ETH_ALEN) == 0)
        {
            ESP_LOGE(TAG_MESSAGER, "Address %u has no peer MAC, skip event %ld", address, event_id);
            xEventGroupClearBits(send_state, SEND_BUSY);
            return;
        }
...
esp_err_t ESPNowProtocol::reset_armour_id()
{
    ...
    // 等待全部击中，0b11111；超时后回滚，避免系统长期停在半初始化状态
    const EventBits_t expected_bits = 0b11111;
    const TickType_t reset_timeout = pdMS_TO_TICKS(15000);
    EventBits_t hit_bits = xEventGroupWaitBits(reset_armour_id_event_group, expected_bits, pdFALSE, pdTRUE, reset_timeout);
    if ((hit_bits & expected_bits) != expected_bits)
    {
        ESP_LOGE(TAG_MESSAGER, "[Reset ID] Timeout waiting all hits, bits=0x%02X", (unsigned int)hit_bits);
        bool first_missing = true;
        char missing_log[64] = {0};
        size_t pos = 0;
        pos += snprintf(missing_log + pos, sizeof(missing_log) - pos, "[Reset ID] Missing armour:");
        for (uint8_t i = 0; i < 5; i++)
        {
            if ((hit_bits & (1 << i)) == 0)
            {
                pos += snprintf(missing_log + pos, sizeof(missing_log) - pos, "%s %u", first_missing ? "" : ",", i + 1);
                first_missing = false;
                if (pos >= sizeof(missing_log))
                {
                    break;
                }
            }
        }
        ESP_LOGE(TAG_MESSAGER, "%s", missing_log);
        // 停止所有装甲，避免部分设备保持常亮
...
// PING EVENT 数据
...
#if CONFIG_POWERRUNE_TYPE == 1
    // 快速清理建链期间残留包，避免在持续PING场景下阻塞后续BLE初始化
    const TickType_t drain_window = pdMS_TO_TICKS(300);
    const uint16_t max_drain_packets = 32;
    const TickType_t drain_start = xTaskGetTickCount();
    uint16_t drained_packets = 0;
    while ((xTaskGetTickCount() - drain_start) < drain_window && drained_packets < max_drain_packets)
    {
        if (xQueueReceive(espnow_rx_queue, received_data, 0) != pdTRUE)
        {
            break;
        }
        free(received_data->data);
    ...
    ESP_LOGI(TAG_MESSAGER, "Peer queue drain complete: %u packet(s)", drained_packets);
#endif
    free(received_data);
    ...
    for (uint8_t i = 0; i < 6; i++)
    {
        if (memcmp(mac_addr[i], NULL_mac, ESP_NOW_ETH_ALEN) == 0)
            ESP_LOGW(TAG_MESSAGER, "Peer slot %u empty", i);
        else
            ESP_LOGI(TAG_MESSAGER, "Peer slot %u => " MACSTR, i, MAC2STR(mac_addr[i]));
    }
#endif
...
// led闪烁
    ...
    {
        // Do not block app_main here, otherwise BLE init/start may never execute.
