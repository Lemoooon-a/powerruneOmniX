...
static constexpr bool OTA_DISABLED = false; // OTA功能开关，true为关闭，false为开启
...
{ // 常亮LED
    ...
    if (!OTA_DISABLED && config->get_config_common_info_pt()->auto_update)
    {
        // Start OTA
    ...
    else if (OTA_DISABLED)
    {
        ESP_LOGI(TAG_FIRMWARE, "OTA disabled by firmware setting.");
    }
...
void Firmware::global_pr_event_handler(void *handler_arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    if (event_base == PRC)
    {
        ...
        case OTA_BEGIN_EVENT:
        {
            if (OTA_DISABLED)
            {
                ESP_LOGW(TAG_FIRMWARE, "OTA_BEGIN_EVENT ignored: OTA disabled.");
                break;
            }
