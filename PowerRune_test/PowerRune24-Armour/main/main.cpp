...
extern "C" void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Ring-1 macro test mode enabled: Firmware/ESP-NOW are disabled");
...
#if CONFIG_ARMOUR_LOCAL_HIT_TEST_MODE
    ESP_LOGI(TAG, "Local hit test mode enabled: WiFi/ESP-NOW/Firmware are disabled");
    led->set_mode(LED_MODE_BLINK, 0);

    // 进入空闲等待击打状态，命中后直接显示对应环数
    PRA_STOP_EVENT_DATA pra_stop_event_data = {};
    pra_stop_event_data.address = 0xFF;
