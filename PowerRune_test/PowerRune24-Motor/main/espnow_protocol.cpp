...
{
    mac_to_address_map.clear();
    mac_address_t mac_address;
    for (uint8_t i = 0; i < 6; i++)
    {
        // Skip empty slots so unresolved address won't be mapped to a fake index.
...
// ID校验
            ...
            if (espnow_data_pack->event_id == RESPONSE_EVENT && espnow_data_pack->event_base[2] == 'C') // PRC:RESPONSE_EVENT
            {
                ...
                {
                    esp_err_t led_err = led->set_mode(LED_MODE_BLINK, config->get_config_info_pt()->armour_id);
                    if (led_err != ESP_OK)
                        ESP_LOGW(TAG_MESSAGER, "Failed to set LED blink mode: %s", esp_err_to_name(led_err));
                }
                else
                {
                    esp_err_t led_err = led->set_mode(LED_MODE_BLINK, 0);
                    if (led_err != ESP_OK)
                        ESP_LOGW(TAG_MESSAGER, "Failed to set LED blink mode: %s", esp_err_to_name(led_err));
                }
#endif
#if CONFIG_POWERRUNE_TYPE == 2 // Motor
                // 开启led fade
                esp_err_t led_err = led->set_mode(LED_MODE_FADE, 0);
                if (led_err != ESP_OK)
                    ESP_LOGW(TAG_MESSAGER, "Failed to set LED fade mode: %s", esp_err_to_name(led_err));
#endif
                // 更新ID
