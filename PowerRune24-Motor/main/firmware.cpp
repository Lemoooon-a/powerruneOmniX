...
#if CONFIG_POWERRUNE_TYPE == 2 // MOTORCTL
    // 自愈历史异常配置：若PID/限幅明显异常，回退默认值
    if (config_info.kp <= 0.0f || config_info.out_max < 1000.0f || config_info.motor_num == 0)
    {
        ESP_LOGW(TAG_FIRMWARE,
                 "Invalid motor config detected (kp=%.3f, out_max=%.3f, motor_num=%u), restoring defaults",
                 config_info.kp, config_info.out_max, config_info.motor_num);
        config_info = {
            .kp = 4.0,
            .ki = 0.3,
            .kd = 0.6,
