...
[[maybe_unused]] static esp_err_t add_peer_if_needed(const esp_now_peer_info_t *peer)
{
...
{
    ESP_LOG_BUFFER_HEX_LEVEL(TAG_MESSAGER, data, len, ESP_LOG_DEBUG);
}
...
#endif
    {
        ESP_LOGD(TAG_MESSAGER, "Receive cb MAC " MACSTR " ignored", MAC2STR(esp_now_info->des_addr));
        return;
    }
...
// 包有效，更新ID
        ...
        else if (header == POWERRUNE_ACK_OK_HEADER && (xEventGroupGetBits(send_state) & SEND_ACK_PENDING_BIT))
        {
            ...
            {
                ESP_LOGE(TAG_MESSAGER, "ACK_OK packet length %u invalid", received_data->len);
                free(received_data->data);
                continue;
            }
            // 这是一个ACK_OK包,
            ...
            ack_ok_pack.pack_id = (data[3] << 8) | data[2];
        ...
        else if (header == POWERRUNE_ACK_FAIL_HEADER && (xEventGroupGetBits(send_state) & SEND_ACK_PENDING_BIT))
        {
            if (received_data->len < 4)
            {
                ESP_LOGE(TAG_MESSAGER, "ACK_FAIL packet length %u invalid", received_data->len);
                free(received_data->data);
                continue;
            }
            // 这是一个ACK_FAIL包
            ...
            ack_fail_pack.pack_id = (data[3] << 8) | data[2];
...
esp_err_t ESPNowProtocol::send_data(uint8_t *dest_mac, esp_event_base_t event_base, int32_t event_id, void *data, uint16_t data_len, uint8_t wait_ack, uint8_t mutex, uint8_t id_plus)
{
    ...
    */
    ...
    if (data_len > sizeof(packet->event_data) || (data_len > 0 && data == NULL))
    {
        ESP_LOGE(TAG_MESSAGER, "Invalid send_data args, len=%u", data_len);
        free(packet);
...
void ESPNowProtocol::tx_event_handler(void *handler_args, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    if (peer_list_established)
    {
        ...
        if (event_data == NULL)
        {
            ESP_LOGE(TAG_MESSAGER, "event_data is NULL for event_id %ld", (long)event_id);
...
esp_err_t ESPNowProtocol::establish_peer_list(uint8_t *response_mac)
{
    ...
    esp_err_t add_broadcast_err = add_peer_if_needed(peer);
    if (add_broadcast_err != ESP_OK)
    {
        ESP_LOGE(TAG_MESSAGER, "Failed to add broadcast peer: %s", esp_err_to_name(add_broadcast_err));
        free(peer);
        return add_broadcast_err;
    }
#else
    bool reset_id = false;
#endif
...
// PING EVENT 数据
        ...
        if (header == POWERRUNE_DATA_HEADER)
        {
            ...
            esp_err_t add_peer_err = add_peer_if_needed(peer);
            if (add_peer_err != ESP_OK)
            {
                ESP_LOGE(TAG_MESSAGER, "Peer add failed: %s", esp_err_to_name(add_peer_err));
                free(received_data->data);
                continue;
            }
            // 写入mac_addr
            ...
            {
                ESP_LOGD(TAG_MESSAGER, "PING packet %i event_base check failed", packet.pack_id);
                // 释放内存
                free(received_data->data);
                continue;
            }
            ...
            {
                ESP_LOGD(TAG_MESSAGER, "PING packet %i event_id check failed", packet.pack_id);
                // 释放内存
                free(received_data->data);
                continue;
            }
...
#if CONFIG_POWERRUNE_TYPE == 1
    // 快速清空队列，避免在设备持续发包时永久阻塞初始化流程
    while (xQueueReceive(espnow_rx_queue, received_data, 0) == pdTRUE)
    {
        free(received_data->data);
    }
#endif
...
{
    // 初始化espnow，PMK和LMK
    esp_err_t now_init_err = esp_now_init();
    if (now_init_err != ESP_OK && now_init_err != ESP_ERR_ESPNOW_EXIST)
        ESP_ERROR_CHECK(now_init_err);

    esp_err_t wifi_start_err = esp_wifi_start();
    if (wifi_start_err != ESP_OK && wifi_start_err != ESP_ERR_INVALID_STATE && wifi_start_err != ESP_ERR_WIFI_CONN)
        ESP_ERROR_CHECK(wifi_start_err);

    esp_err_t set_channel_err = esp_wifi_set_channel(CONFIG_ESPNOW_CHANNEL, WIFI_SECOND_CHAN_NONE);
    if (set_channel_err != ESP_OK)
        ESP_LOGW(TAG_MESSAGER, "esp_wifi_set_channel failed: %s", esp_err_to_name(set_channel_err));
    // set pmk
