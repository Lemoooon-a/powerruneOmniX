...
// BLOB
    ...
    size_t required_size_armour = sizeof(config_armour_info);
    size_t required_size_motor = sizeof(PowerRune_Motor_config_info_t);
#endif
#if CONFIG_POWERRUNE_TYPE == 2 // MOTORCTL
...
#if CONFIG_POWERRUNE_TYPE == 1
    // 兼容旧版本：旧版本未持久化装甲与电机配置，缺失则回填默认值
    ret = nvs_get_blob(my_handle, "pr_arm_cfg", &config_armour_info, &required_size_armour);
    if (ret == ESP_ERR_NVS_NOT_FOUND)
    {
        for (uint8_t i = 0; i < 5; i++)
        {
            config_armour_info[i].brightness = 127;
            config_armour_info[i].armour_id = i + 1;
            config_armour_info[i].brightness_proportion_matrix = 127;
            config_armour_info[i].brightness_proportion_edge = 127;
        }
        need_save = true;
    }
    else
    {
        ESP_GOTO_ON_ERROR(ret, error, TAG_FIRMWARE, "Failed to read armour config from NVS flash");
    }

    ret = nvs_get_blob(my_handle, "pr_mtr_cfg", &config_motor_info, &required_size_motor);
    if (ret == ESP_ERR_NVS_NOT_FOUND)
    {
        config_motor_info = {
            .kp = 4.0,
    ...
    {
        ESP_GOTO_ON_ERROR(ret, error, TAG_FIRMWARE, "Failed to read motor config from NVS flash");
    }
#endif
    // Close
    nvs_close(my_handle);
    ...
    if (need_save)
    {
        ESP_LOGW(TAG_FIRMWARE, "Legacy RLogo config detected, writing default armour/motor config");
        return Config::save();
    }
#endif
    return ESP_OK;
...
#if CONFIG_POWERRUNE_TYPE == 1 // RLOGO
    ESP_ERROR_CHECK(nvs_set_blob(my_handle, "pr_arm_cfg", &config_armour_info, required_size_armour));
    ESP_ERROR_CHECK(nvs_set_blob(my_handle, "pr_mtr_cfg", &config_motor_info, required_size_motor));
#endif
...
esp_err_t Config::reset()
{
    ...
    for (uint8_t i = 0; i < 5; i++)
    {
        config_armour_info[i] = {
            .brightness = 127,
            .armour_id = (uint8_t)(i + 1),
            .brightness_proportion_matrix = 127,
            .brightness_proportion_edge = 127,
        };
    }
...
esp_err_t Firmware::wifi_connect(const PowerRune_Common_config_info_t *config_common_info, uint8_t retryNum)
{
    ...
    wifi_config_t wifi_config = {
        .sta = {
            ...
            .threshold = {
                .rssi = -127,
                .authmode = strlen(config_common_info->SSID_pwd) == 0 ? WIFI_AUTH_OPEN : WIFI_AUTH_WPA2_PSK,
            },
        },
    };
